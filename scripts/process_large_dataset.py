...
def process_chunks(chunk_files: List[Path], output_file: Path, command: str, target_output_dir: Path, logger: Logger, config: dict):
...
def main():
    ...
    # Process each chunk
    process_chunks(chunk_files, args.output, "validation", args.target_output_dir, logger, config)
