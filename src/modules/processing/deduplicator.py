...
from src.common.utils import ensure_dir, configure_proxy_session

urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)

def download_and_hash_fast(url: str, timeout: int, config: Dict) -> tuple[str, str | None]:
...
def run(args: Any, config: Dict, logger: Logger, workflow_data: Dict) -> Dict:
    ...
    with tqdm(total=len(live_urls), desc=f"[{target}] Deduplicating", unit="url", leave=False) as pbar:
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            future_to_url = {
                executor.submit(download_and_hash_fast, url, timeout, config): url 
